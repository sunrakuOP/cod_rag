...
- [ ] Métricas: confirmación %, no-shows antes/después, pedidos/día automáticos (v1 con confirmación % + pedidos/día ya corriendo, `npm run report:metrics` — falta comparación antes/después y costo por confirmación).
...
- **Reporte de métricas — v1, dejado a propósito sin terminar (2026-08-13).** `npm run report:metrics [-- --client=<slug>]` (`src/scripts/reportMetrics.ts`, consulta directa a `orders`/`messages`, sin endpoint HTTP): pedidos por estado, tasa de confirmación, mensajes por estado, pedidos/día. Probado local con datos reales de las pruebas de esta sesión. **Falta para la próxima:** comparación no-shows antes/después (necesita una línea base externa, no solo una query), costo por confirmación (`messages.cost_estimate` ya se puede sumar, no está conectado al reporte todavía), y evaluar si conviene un endpoint HTTP en vez de solo consola.
