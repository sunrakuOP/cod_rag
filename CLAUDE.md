...
- **⚠️ Importante: Railway NO redeploya solo con `git push` a GitHub.** El servicio no tiene integración de auto-deploy conectada — el historial de deploys en el dashboard solo muestra "railway up via CLI". Confirmado el 2026-08-13: el código de `/api/metrics` y el fix de auth de `/api/test-orders` estaban pusheados a `origin/main` hacía rato, pero el servicio seguía sirviendo un build de ~12hs antes hasta correr `railway up` manualmente. Agregar/cambiar una env var en el dashboard sí dispara un redeploy, pero **del último build subido**, no del código nuevo. Después de cualquier cambio de código: `railway up` (CLI, ya vinculado al proyecto — `railway status` lo confirma) antes de asumir que producción lo tiene.
